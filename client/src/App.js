import { useState, useEffect } from "react";
import io from "socket.io-client";
import Editor from "@monaco-editor/react";
import "./App.css";

// --- SMART CONNECTION LOGIC ---
// 1. Check if we are running on localhost
const isLocal = window.location.hostname === "localhost";

// 2. Set the URL automatically
// REPLACE "https://my-code-collab.onrender.com" with your REAL Render URL later!

const SERVER_URL = isLocal 
  ? "http://localhost:3001" 
  : "https://my-code-collab.onrender.com";

const socket = io(SERVER_URL, {
  transports: ["websocket", "polling"],
  withCredentials: true
});
  // ... rest of your code ...
function App() {
  const [room, setRoom] = useState("");
  const [username, setUsername] = useState("");
  const [code, setCode] = useState("// Write your code here...");
  const [language, setLanguage] = useState("javascript");
  const [isJoined, setIsJoined] = useState(false);
  const [notification, setNotification] = useState("");

  // --- 1. SOCKET LISTENERS ---
  useEffect(() => {
    socket.on("receive_code", (data) => {
      setCode(data);
    });

    socket.on("notification", (msg) => {
      setNotification(msg);
      setTimeout(() => setNotification(""), 3000);
    });
  }, []);

  // --- 2. FUNCTIONS ---
  const joinRoom = () => {
    if (room !== "" && username !== "") {
      socket.emit("join_room", { room, username });
      setIsJoined(true);
    } else {
      alert("Please enter both a Room ID and a Username!");
    }
  };

  const generateRoomId = () => {
    setRoom(Math.random().toString(36).substring(7));
  };

  const copyRoomId = () => {
    navigator.clipboard.writeText(room);
    setNotification("ID COPIED!");
  };

  const handleEditorChange = (value) => {
    setCode(value);
    socket.emit("code_change", { code: value, room });
  };

  const handleLanguageChange = (e) => {
    setLanguage(e.target.value);
  };

  // --- 3. THE RESPONSIVE DESIGN (JSX) ---
  return (
    <div className="App" style={{
      minHeight: "100vh",
      backgroundColor: "#000000",
      color: "white",
      fontFamily: "'Courier New', Courier, monospace",
      display: "flex",
      flexDirection: "column",
      alignItems: "center",
      padding: "10px",         // Added padding for mobile edges
      overflowX: "hidden"      // Prevents horizontal scrollbar on phones
    }}>

      {/* HEADER - Uses Clamp for responsive font size */}
      <h1 style={{
        marginTop: "30px",
        fontSize: "clamp(2rem, 5vw, 3rem)", // Scales between 2rem and 3rem depending on screen width
        letterSpacing: "5px",
        textShadow: "2px 2px #333",
        textAlign: "center"
      }}>
        CODE COLLAB
      </h1>

      {!isJoined ? (
        // --- LOGIN SCREEN (Responsive) ---
        <div style={{
          marginTop: "50px",
          backgroundColor: "#000",
          border: "2px solid white",
          padding: "30px",
          width: "100%",           // Full width on phones
          maxWidth: "400px",       // Limit width on desktop
          textAlign: "center",
          boxShadow: "10px 10px 0px white",
          boxSizing: "border-box"  // Ensures padding doesn't break width
        }}>
          <h3 style={{ marginBottom: "30px", fontSize: "1.2rem", textTransform: "uppercase" }}>Enter The Room</h3>

          <div style={{ marginBottom: "20px", textAlign: "left" }}>
            <label style={{ display: "block", marginBottom: "8px", fontSize: "0.9rem", fontWeight: "bold" }}>USERNAME</label>
            <input
              onChange={(e) => setUsername(e.target.value)}
              style={{ width: "100%", padding: "12px", backgroundColor: "black", border: "1px solid white", color: "white", fontFamily: "inherit", fontSize: "1rem", outline: "none", boxSizing: "border-box" }}
            />
          </div>

          <div style={{ marginBottom: "40px", textAlign: "left" }}>
            <label style={{ display: "block", marginBottom: "8px", fontSize: "0.9rem", fontWeight: "bold" }}>ROOM ID</label>
            <div style={{ display: "flex", gap: "10px" }}>
              <input
                value={room}
                onChange={(e) => setRoom(e.target.value)}
                style={{ flex: 1, padding: "12px", backgroundColor: "black", border: "1px solid white", color: "white", fontFamily: "inherit", fontSize: "1rem", outline: "none", minWidth: "0" }}
              />
              <button onClick={generateRoomId} style={{ padding: "0 20px", backgroundColor: "white", color: "black", border: "none", fontWeight: "bold", cursor: "pointer", fontFamily: "inherit" }}>GEN</button>
            </div>
          </div>

          <button onClick={joinRoom} style={{ width: "100%", padding: "15px", backgroundColor: "white", color: "black", border: "none", fontSize: "1.2rem", fontWeight: "bold", cursor: "pointer", letterSpacing: "2px", fontFamily: "inherit", textTransform: "uppercase" }}>Join Room</button>
        </div>
      ) : (
        // --- EDITOR SCREEN (Responsive) ---
        <div style={{
          width: "100%",
          maxWidth: "1200px", // Prevents editor from stretching too wide on huge screens
          marginTop: "20px"
        }}>

          {/* INFO BAR - Wraps on small screens */}
          <div style={{
            display: "flex",
            justifyContent: "space-between",
            alignItems: "center",
            marginBottom: "10px",
            borderBottom: "1px solid white",
            paddingBottom: "8px",
            flexWrap: "wrap", // Allows items to stack on phones
            gap: "10px"
          }}>

            <div style={{ display: "flex", gap: "15px", alignItems: "center", flexWrap: "wrap" }}>
              <span style={{ whiteSpace: "nowrap" }}>ROOM: <b>{room}</b> <button onClick={copyRoomId} style={{ marginLeft: "5px", background: "none", border: "1px solid white", color: "white", cursor: "pointer", padding: "0px 5px", fontSize: "0.7rem" }}>COPY</button></span>
              <span style={{ whiteSpace: "nowrap" }}>USER: <b>{username}</b></span>

              <select
                value={language}
                onChange={handleLanguageChange}
                style={{
                  backgroundColor: "black",
                  color: "white",
                  border: "1px solid white",
                  padding: "2px 5px",
                  fontFamily: "inherit",
                  cursor: "pointer",
                  outline: "none"
                }}
              >
                <option value="javascript">JS</option>
                <option value="cpp">C++</option>
                <option value="python">PY</option>
                <option value="java">Java</option>
              </select>
            </div>

            <div style={{ color: "#0f0", fontSize: "0.8rem", fontWeight: "bold", opacity: notification ? 1 : 0, transition: "opacity 0.3s ease" }}>
              {notification ? `> ${notification}` : ""}
            </div>
          </div>

          {/* MONACO EDITOR CONTAINER */}
          <div style={{ border: "1px solid white", height: "70vh" }}>
            <Editor
              height="100%"
              language={language}
              theme="vs-dark"
              value={code}
              onChange={handleEditorChange}
              options={{
                minimap: { enabled: false }, // Disabling minimap saves space on mobile
                fontSize: 14, // Slightly smaller font for mobile readability
                wordWrap: "on", // Essential for mobile so text doesn't hide
                padding: { top: 20 },
                automaticLayout: true // Crucial: Makes editor resize when screen rotates
              }}
            />
          </div>

        </div>
      )}
    </div>
  );
}

export default App;